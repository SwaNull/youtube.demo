...
function lineTest(tester) {
	...
	function success() {
		$('#Stage_snap').off('mousedown mouseup mousemove');
...
function newGame01() {
	...
	$('#Stage_snap').detach().appendTo('#Stage'); // Reorder the Snap stage, placing it on top of the stacking order
	...
	$('#Stage_snap').mousemove(function drawLine(e) {
		if (dragging) {
			testx2 = e.pageX - (Math.floor($(this).position().left));
			testy2 = e.pageY - (Math.floor($(this).position().top));
			dx = testx2 - testx1;
			dy = testy2 - testy1;
			if (e.shiftKey) {
				slope = (dy / dx);
